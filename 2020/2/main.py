...
def main(inp):
    ...
    print(sum(lo <= pwd.count(ch) <= hi for lo, hi, ch, pwd in parse_pwds(inp)))
    # Part 2
    print(sum(max(i, k) <= len(pwd) and (pwd[i-1], pwd[k-1]).count(ch) == 1 for i, k, ch, pwd in parse_pwds(inp)))
