...
def main(inp):
    # Part 1
    s = set()
    for l in filter(None, (list(l) for l in inp.splitlines())):
        x, y = 0, 0
        # Grid is represented by an 'infinite parallelogram'
        #    /
        # __/___
        #  /
        while len(l) > 0:
    ...
    for _ in range(100):
        t = set()
        for x, y in s:
            if 0 < sum((x + dx, y + dy) in s for dx, dy in ((1, 0), (1, -1), (0, -1), (-1, 0), (-1, 1), (0, 1))) < 3:
                t.add((x, y))
            for a, b in ((1, 0), (1, -1), (0, -1), (-1, 0), (-1, 1), (0, 1)):
                if sum((x + a + dx, y + b + dy) in s for dx, dy in ((1, 0), (1, -1), (0, -1), (-1, 0), (-1, 1), (0, 1))) == 2:
                    t.add((x + a, y + b))
        s = t
    print(len(s))
